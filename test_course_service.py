...
def test_create_course():
    ...
    ) == 1, "Create Course test failed"
    assert (
        course_service.create_course("Introduction to Python")
    ) == 2, "Create Course test failed"
...
def test_get_course_by_id():
    ...
    assert (
        course_service.get_course_by_id(1)[0]["name"] == "Introduction to C++"
    ), "Get Course by id test failed"
    assert (
        course_service.get_course_by_id(2)[0]["name"] == "Introduction to Python"
    ), "Get Course by id test failed"
...
def test_enroll_dropout_student():
    ...
    ) == True, "Enroll Dropout Student test failed"
    assert (
        course_service.enroll_student(2, 1)
    ) == False, "Enroll Dropout Student test failed"
    assert (
        course_service.dropout_student(1, 1)
    ) == True, "Enroll Dropout Student test failed"
    assert (
        course_service.dropout_student(1, 1)
    ) == False, "Enroll Dropout Student test failed"
...
def test_create_assignment():
    ...
    ) == 0, "Create Assignment test failed"
    course_service.create_course("Introduction to C++")
    assert (
        course_service.create_assignment(1, "Assignment 1")
    ) == 1, "Create Assignment test failed"


def test_submit_assignment():
    course_service = CourseServiceImpl()
    assert (
        course_service.submit_assignment(1, 1, 1, 100)
    ) == False, "Submit Assignment test failed"
    course_service.create_course("Introduction to C++")
    assert (
        course_service.submit_assignment(1, 1, 1, 100)
    ) == False, "Submit Assignment test failed"
    ...
    assert (
        course_service.submit_assignment(1, 1, 1, 100)
    ) == True, "Submit Assignment test failed"
...
def test_get_grades():
    ...
    course_service.create_course("Introduction to C++")
    assert (
        course_service.get_student_grade_avg(2, 1)
    ) == "Student or Course Doesn't exist", "Test get_student_grade_avg failed"
    assert (
        course_service.get_assignment_grade_avg(1, 1)
    ) == "There are no grades for this assignment", (
        "Test get_assignment_grade_avg failed"
    )
    assert (
        course_service.get_student_grade_avg(1, 1)
    ) == "Student or Course Doesn't exist", "Test get_student_grade_avg failed"
    course_service.create_assignment(1, "Assignment 1")
    ...
    assert (
        course_service.get_student_grade_avg(1, 1)
    ) == 0, "Test get_student_grade_avg failed"
    ...
    assert (
        course_service.get_student_grade_avg(1, 1)
    ) == 30, "Test get_student_grade_avg failed"
    assert (
        course_service.get_assignment_grade_avg(1, 1)
    ) == 16, "Test get_assignment_grade_avg failed"
    assert (course_service.get_top_five_students(1)) == [
        5,
        2,
        3,
        1,
        4,
    ], "Test get_top_five_students failed"
    assert (course_service.get_top_five_students(2)) == [
        0
    ], "Test get_top_five_students failed"
    assert (
        course_service.get_student_marks_for_assignment(1, 6, 3)
    ) == 7, "Test get_student_marks_for_assignment failed"
    assert (
        course_service.get_student_marks_for_assignment(1, 6, 7)
    ) == "Grades for this assignment doesn't exist", (
        "Test get_student_marks_for_assignment failed"
    )
